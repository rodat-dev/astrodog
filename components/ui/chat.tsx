...
export function AstrodogChat() {
  ...
  return (
    <Card
      className={`pointer-events-auto m-auto flex w-full flex-col items-center justify-center border-none [box-shadow:inset_0_0_6px_var(--violet-8)] ${cardStyles.glassy} transition-all duration-500 md:w-[60dvw] ${messages.length > 0 ? "h-full" : "h-fit"}`}
    >
      <div
        className={`flex flex-col gap-4 p-8 ${messages.length > 0 ? "h-full" : ""}`}
      >
        <TitleWithGradient>Chat with Astrodog</TitleWithGradient>
        <p className="text-center text-sm text-muted-foreground">
          Dog Sitting Manchester&apos;s{" "}
          <span className="font-bold text-violet-500">AI</span> assistant
        </p>
